...
namespace Strike {
    ...
    {
        const auto& lights = mRenderer.getPointLights();
    ...
    void LightCullingPass::buildClusters(const CameraRenderData& cameraData)
    {
        ...
        mClusterBuildShader->setUVec2 ("uScreenDimensions",  glm::uvec2(viewportWidth, viewportHeight));
    ...
    void LightCullingPass::cullLights(const CameraRenderData& cameraData)
    {
        ...
        mLightCullShader->setMat4 ("uViewMatrix", cameraData.camera.getViewMatrix());
    ...
    {
        if (mLightSSBO)   { glDeleteBuffers(1, &mLightSSBO);   mLightSSBO   = 0; }
        if (mClusterSSBO) { glDeleteBuffers(1, &mClusterSSBO); mClusterSSBO = 0; }
    }
}